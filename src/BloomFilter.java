...
public class BloomFilter<E> implements Serializable {
    ...
    /**
     * ESSE CONSTRUTOR NÃO É DA BIBLIOTECA ORIGINAL
     * @param m é o tamanho da assinatura.
     * @param b é a quantidade de bits setados por elemento
     * @param n is the expected number of elements the filter will contain.
     * @param foo é so pra diferenciar a assinatura
     */
    public BloomFilter( int m, int b, int n, boolean foo)
    {
        this.expectedNumberOfFilterElements = n;
        this.k = (int) Math.max(Math.round((bitSetSize / (double) n) * Math.log(2.0)), 1); //usando a estimativa do contrutor abaixo
        this.bitsPerElement = b;
        this.bitSetSize = m;
        numberOfAddedElements = 0;
        this.bitset = new BitSet(bitSetSize);
    }
